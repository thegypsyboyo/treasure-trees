                      ...
                      {filteredCauesItems.map((item, index) => (
                        <div className='portfolio-single portfolio-hover-style' key={index}>       
                              <div className='portfolio-thumb'>
                                ...
                                <Link href={`/donate/donation-details/${item.slug.current}`}  className="tag">
                                  <span>{item.category}</span>
                                </Link>
                              </div>
                              <div className='portfolio-content'>
                                  <span className="portfolio-tag">{item.title}</span>
                                  <Link href={`/donate/donation-details/${item.slug.current}`} className='portfolio-title'>
                                    <span>{item.description.split(" ").slice(0, 10).join(" ")}</span> 
                                  </Link> 
                                  <div className="diation-details">
                                    <div className="progressbar-container">
                                      ...
                                      <div className="give-goal">
                                        <div className="raised">
                                          ...
                                          <p className='donated'> Raised of  ${item.amountTargeted} raised</p>
                                        </div>
                                      </div>
                                    </div>
                                    <div className="project-meta">
                                      <FaCalendarAlt/>
                                      <span>{new Date(item._createdAt).toLocaleDateString("en-Us", {
                                        day: "numeric",
                                        month: "long",
                                        year: "numeric",
                                      })}</span>
                                    ...
                                    <div className="donate-btn">
                                      <Link href={`/donate/donation-details/${item.slug.current}`}>make a donation</Link>
                                    </div> 
                                  </div>                    
                                </div>
                        </div>

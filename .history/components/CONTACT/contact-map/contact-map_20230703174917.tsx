...
const ContactMap = () => {
  ...
  useEffect(() => {
    ...
    const fetchSliderData = async () => {
      try {
        ...
        setMapLocations(response[0].coordinates);
        setIsLoading(false);
  ...
  const extractedLocations = sliderData?.map(item => ({
    name: item.coordinates.name,
    coordinates: [item.coordinates.mapCoordinates.map((data) => data.longitude), item.coordinates.mapCoordinates.map((data) => data.latitude)]
  }));
                ...
                <div className="row">
                  <div className="col-lg-4s">
                    {/* {data.coordinates?.map((item, index) => ( */}
                      <div className="map">
                        {/* <Map lng={item.longitude} lat={item.latitude} zoom={10} />                 */}
                        {/* <Map locations={mapLocations} zoom={7} />                 */}
                      </div>
                    {/* ))} */}
                  </div>
                  <div className="col-lg-4s">

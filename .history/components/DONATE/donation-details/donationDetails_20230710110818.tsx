...
const DonationDetails = ({causesData}: Props) => {
    ...
    //       if (stripeWindow) {
    //         // Add an event listener to detect when the new window/tab is closed
    //         stripeWindow.addEventListener('beforeunload', () => {
    //           setIsLoading(false); // Set loading state back to false when the new window/tab is closed
    //         });
    //       } else {
    //         console.error('Failed to open Stripe checkout in a new window/tab.');
    //         setIsLoading(false); // Set loading state back to false if opening the new window/tab failed
    //       }
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (stripe) {
            ...
            // Redirect to the Stripe checkout page in a new window/tab
            window.open(session.url, '_blank');
        
            setIsLoading(false); // Set loading state back to false after the redirect has been triggered
          } else {
            console.error('Stripe has not yet loaded.');
            setIsLoading(false);
          }
                            ...
                            <div className="donation-container">
                                ...
                                <form className='donate' onSubmit={handleSubmit}>
                                    <div className="form-block">
                                        ...
                                        <div className="form-group">
                                            <label className="custom-donation">OR, ENTER YOUR DONATION
                                            </label>
                                        ...
                                        <button 
                                            ...
                                            type='submit' 
                                        >
                                        {isLoading ? 'Loading...' : 'MAKE A DONATION ❤'}
                                        </button>
                                    </div>
                                </form>
                            </div>
